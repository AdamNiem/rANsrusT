...
use ransrust::{codebook, ANSCoder, ANSDecoder};
...
fn huffman_benchmark_test() {
    ...
    for _ in 0..5 {
        ...
        let now = Instant::now();
        for symbol in data.iter() {
            book.encode(&mut encoded, symbol).unwrap();
        }
        let dur = now.elapsed();
        println!(
            "\t{:.3} seconds elapsed, {:.3}MiB/sec",
    ...
    println!("Encoded size: {} bytes ({} bits)", encoded.len() / 8, encoded.len());
    println!("Compression ratio: {:.3}",
             (data.len()*8) as f64 / encoded.len() as f64
    );

    // Decode and verify
    let decoded: Vec<u8> = tree.decoder(encoded.iter(), data.len()).collect();
